...
func TestTemplateNewWithOptionalParams(t *testing.T) {
	...
	_, err := client.Templates.New(context.TODO(), zavudev.TemplateNewParams{
		...
		Buttons: []zavudev.TemplateNewParamsButton{{
			Type:          "quick_reply",
			Example:       zavudev.String("ORD-12345"),
			OtpType:       "COPY_CODE",
		...
		Footer:                zavudev.String("footer"),
		HeaderContent:         zavudev.String("headerContent"),
		HeaderType:            zavudev.TemplateNewParamsHeaderTypeText,
		InstagramBody:         zavudev.String("instagramBody"),
		SMSBody:               zavudev.String("smsBody"),
		TelegramBody:          zavudev.String("telegramBody"),
...
func TestTemplateSyncWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Templates.Sync(context.TODO(), zavudev.TemplateSyncParams{
		SenderID: zavudev.String("sender_12345"),
	})
	if err != nil {
		var apierr *zavudev.Error
		if errors.As(err, &apierr) {
