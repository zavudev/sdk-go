...
func TestNumber10dlcCampaignPhoneNumberList(t *testing.T) {
	...
	_, err := client.Number10dlc.Campaigns.PhoneNumbers.List(context.TODO(), "campaignId")
	if err != nil {
...
func TestNumber10dlcCampaignPhoneNumberAssign(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
...
func TestNumber10dlcCampaignPhoneNumberUnassign(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
